...
def buildUserAccFeatureMatrix(tripObject, dataSetCollection, labelList):
    #TODO stop hardcoding the range
    ...
    for dataSet in dataSetCollection:
        ...
        #TODO DPCR CODE there is sometimes a difference between gyro and UA values, this is a temporary fix
        ...
        #it looks ugly but it works, I feel like anything else would just make it look more complicated
        #temp is just to know how much texting/watching data I am adding at a time 
...
print("--- %s seconds ---" % (time.time() - start_time))
print("Building training feature matrix")
#featureMatrixTest, testLabelList = buildUserAccFeatureMatrix(test2, imsCarMay25SetTrain, testLabelList)
...
print("--- %s seconds ---" % (time.time() - start_time))
#tRAININGSet        trainingSet1 testingSet1
#testingSet        trainingSet2 testingSet2
#megaSet restSet mixedSet    trainingSet3 testingSet3
#print(clf.predict(buildUserAccFeatureMatrix(test1, trainingSet)))
print("Testing Naive Bayes classifier results")
testingResults = clf.predict(featureMatrixTrain)
#probResults = clf.predict_proba(featureMatrixTest)
#print(probResults)
print("--- %s seconds ---" % (time.time() - start_time))
